    ...
    ZUI.draw = function (timeStamp) {
        ...
        if (timeStamp - ZUI.lastTimeStamp > 1000 / ZUI.frameRate) {
            ...
            if (ZUI.activeView._private.isUpdated) {
                ...
                // call view's post-draw method for drawing things on top of the auto-drawn objects
                ZUI.activeView.postDraw();
...
(function() {
    ...
    // custom post-draw callback (abstract)
    ZUI.View.prototype.postDraw = function() {};
...
(function() {
    ...
    ZUI.RenderedObject.Text.prototype.render = function () {
        ...
        this._private.context.save();
        this._private.context.font = ((this.isBold) ? 'bold ' : '') + ((this.isItalic) ? 'italic ' : '') + '120' + 'px ' + this.font;
        for (var n = 0; n < this._private.lines.length; n++) {
            var line = this._private.lines[n];
            line.renderedWidth = this._private.context.measureText(line.content).width / 120 * this.renderedSize;
        ...
        for (var n = 0; n < this._private.lines.length; n++) {
            ...
            this._private.context.scale(this.renderedSize / 120 * this.hStretch, this.renderedSize / 120 * this.vStretch);
            if (this.stroke) {
        ...
        for (var n = 0; n < this._private.lines.length; n++) {
            ...
            this._private.context.lineTo(line.renderedPosition.x, line.renderedPosition.y);
        }
        this._private.context.restore();
...
(function() {
    ...
    ZUI.RenderedObject.LinePath.prototype.render = function () {
        ...
        for (var n = 0; n < this.vertices.length; n++) {
            this.renderedVertices.push({
                x: ZUI.Helper.interpretScale(this.vertices[n].x, this.verticesScale[n]),
                y: ZUI.Helper.interpretScale(this.vertices[n].y, this.verticesScale[n])
            });
        }
